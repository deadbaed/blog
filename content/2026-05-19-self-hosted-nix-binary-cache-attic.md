...
If you intend to have your cache open to the internet, put it behind a reverse proxy with a TLS certificate. I personally use nginx, it works great and thanks to nix I can configure it without being an expert in the nginx configuration format.
...
- As the administrator, you run `atticd-atticadm` on the server running `atticd` to generate tokens which you give to users. When generating a token, you befine what the token can do.
